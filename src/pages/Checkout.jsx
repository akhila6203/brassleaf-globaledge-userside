...
// import { useAuth } from "../context/AuthContext";
// import AddressForm from "../components/AddressForm";
// import { EMPTY_ADDRESS, getDefaultAddress, readSaved, saveBoth, saveSharedAddress } from "../utils/addressStorage";

// const labels = {
//   firstName: "Billing Student First Name",
//   lastName: "Billing Student Last Name",
//   address: "Billing Street address",
//   city: "Billing Town / City",
//   pincode: "Billing PIN Code",
//   phone: "Billing Phone",
...
//   useEffect(() => {
//     const profile = readSaved("uniforms_profile", {});
//     const savedAddress = getDefaultAddress();
...
//       ...EMPTY_ADDRESS,
//       ...(savedAddress || {}),
//       firstName: savedAddress?.firstName || profile?.firstName || names[0] || "",
//       lastName: savedAddress?.lastName || profile?.lastName || names.slice(1).join(" ") || "",
//       email: savedAddress?.email || profile?.email || "",
//       phone: savedAddress?.phone || profile?.phone || "",
//       studentClass: savedAddress?.studentClass || "Nursery",
//       country: savedAddress?.country || "India",
//       state: savedAddress?.state || "Telangana",
//     });
//   }, []);

//   const validate = () => {
//     const next = {};
//     ["firstName", "lastName", "address", "city", "pincode", "phone", "email", "admissionNo", "parentName"].forEach((key) => {
//       if (!String(form[key] || "").trim()) next[key] = `${labels[key]} is a required field.`;
//     });
...
//   // Tax is already included in the product subtotal, matching the BrassLeaf display style.
...
//     // Checkout and Profile use the exact same saved address.
//     const savedAddress = saveSharedAddress(form);
//     const previousProfile = readSaved("uniforms_profile", {});
...
//     const order = {
//       id: `BL${Date.now().toString().slice(-8)}`,
//       subtotal,
//       shipping,
//       total,
//       cgst,
...
//       phone: form.phone,
//       email: form.email,
//       paymentMethod: "Paytm Payment Gateway",
//       notes,
...
//       items: cart.map((item) => ({ key: item.key, id: item.id, name: item.name, image: item.image, size: item.size, quantity: item.quantity, price: item.price })),
//     };

//     const previousOrders = readSaved("uniforms_orders", []);
//     saveBoth("uniforms_orders", [order, ...previousOrders]);
//     saveBoth("uniforms_last_order", order);
//     clearCart();
//     navigate("/order-success", { state: order });
//   };

//   if (!cart.length) {
//     return <main className="container-site py-24 text-center"><h1 className="text-3xl font-black text-[#243346]">Your cart is empty</h1></main>;
//   }
...
//           <div className="mb-9 border-l-4 border-red-700 bg-[#e93b1d] px-6 py-4 text-sm font-bold text-white">
//             {Object.values(errors).map((message, index) => <p key={index} className="py-0.5">{message}</p>)}
//           </div>
//         )}

//         {!isAuthenticated && (
//           <button type="button" onClick={() => navigate("/profile")} className="mb-7 w-full bg-[#243346] px-5 py-3 text-left text-sm font-bold text-white transition hover:bg-[#304154]">
//             Returning customer? Click here to login
//           </button>
//         )}

//         <form onSubmit={submit} noValidate className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_460px]">
//           <section>
//             <h1 className="text-2xl font-black text-[#243346]">Billing & Shipping</h1>
//             <h2 className="mt-5 text-xl font-black text-[#243346]">Global Edge School of Learning</h2>
//             <div className="mt-6">
//               <AddressForm value={form} onChange={setForm} errors={errors} />
//             </div>

//             <h2 className="mt-10 text-xl font-black text-[#243346]">Additional information</h2>
//             <label className="mt-5 block text-sm font-semibold text-[#243346]">Order notes <span className="font-normal text-slate-400">(optional)</span></label>
//             <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={4} className="input-field mt-2 rounded-sm" placeholder="Notes about your order, e.g. special notes for delivery." />
//           </section>

//           <aside className="h-fit border-[5px] border-[#eeeeee] bg-white p-6 lg:sticky lg:top-24">
//             <h2 className="text-2xl font-black text-[#243346]">Your order</h2>
//             <div className="mt-7 flex justify-between border-b border-slate-200 pb-4 text-sm font-black text-[#243346]"><span>Product</span><span>Subtotal</span></div>

//             <div className="divide-y divide-slate-100">
//               {cart.map((item) => (
//                 <div key={item.key} className="flex justify-between gap-5 py-4 text-sm text-[#243346]">
...
//             <div className="flex justify-between border-t border-slate-200 py-5 text-sm text-[#243346]"><b>Subtotal</b><b>₹{subtotal.toFixed(2)}</b></div>
//             <div className="flex justify-between border-t border-slate-200 py-5 text-sm text-[#243346]"><b>Shipping</b><span>Flat rate: <b>₹{shipping.toFixed(2)}</b></span></div>

//             <div className="border-t border-slate-200 py-6">
//               <div className="flex items-start justify-between gap-5">
//                 <b className="pt-2 text-sm text-[#243346]">Total</b>
//                 <div className="text-right">
//                   <div className="text-[26px] font-black text-[#D9A537]">₹{total.toFixed(2)}</div>
//                   <div className="mt-2 text-xs font-semibold leading-6 text-[#243346]">
//                     (includes <strong className="text-[#D9A537]">₹{cgst.toFixed(2)}</strong> 2.5% CGST,<br />
//                     <strong className="text-[#D9A537]">₹{sgst.toFixed(2)}</strong> 2.5% SGST)
//                   </div>
//                 </div>
//               </div>
//             </div>
...
//                 <span className="mt-1 grid h-4 w-4 place-items-center rounded-full border-2 border-[#D9A537]"><span className="h-1.5 w-1.5 rounded-full bg-[#D9A537]" /></span>
//                 <div className="flex-1">
//                   <div className="flex flex-wrap items-center gap-3">
//                     <b className="text-sm text-[#243346]">Paytm Payment Gateway</b>
//                     <span className="inline-flex items-center font-black"><span className="text-[#162d70]">pay</span><span className="text-[#00baf2]">tm</span><span className="ml-1 rounded bg-[#00baf2] px-1.5 py-0.5 text-[10px] text-white">PG</span></span>
//                   </div>
//                   <p className="mt-3 text-xs font-semibold leading-5 text-[#243346]">The best payment gateway provider in India for e-payment through credit card, debit card & netbanking.</p>
//                 </div>
//               </div>
//             </div>

//             <button type="submit" className="btn-gold mt-7 w-full rounded-sm">Place order</button>
//           </aside>
//         </form>
//       </div>
//     </main>
